...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    newTuple = (len(ratings), float(sum(ratings))/len(ratings))
    return (movieID, newTuple)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda x: float(pow(x[1][0]-x[1][1], 2))))
...
myRatedMovies = [
     (myUserID, 2, 2), (myUserID, 22, 5), (myUserID, 47, 3), (myUserID, 110, 3), (myUserID, 294, 5),
     (myUserID, 551, 5), (myUserID, 730, 5), (myUserID, 912, 2), (myUserID, 924, 2), (myUserID, 608, 4),
     (myUserID, 541, 4)
...
myUnratedMoviesRDD = (moviesRDD
                      .subtractByKey(sc.parallelize(myRatedMovies).map(lambda x: (x[1], x[2])))
