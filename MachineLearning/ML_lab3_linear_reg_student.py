...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    uniVector = line.split(',')
    newPoint = LabeledPoint(uniVector[0], uniVector[1:])
    return newPoint
...
def calcRMSE(labelsAndPreds):
    ...
    return np.sqrt(labelsAndPreds.map(lambda (lab, pred): squaredError(lab, pred)).sum()/n)
...
def twoWayInteractions(lp):
    ...
    """
    cartes = itertools.product(lp.features, lp.features)
    newFeatures = [x[0]*x[1] for x in cartes]
    newFeatures = np.hstack((lp.features, newFeatures))
