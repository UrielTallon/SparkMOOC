...
def simpleTokenize(string):
    ...
    """
    return [x.lower() for x in re.split(split_regex, string) if x!= '']
...
def tokenize(string):
    ...
    """
    return [x.lower() for x in re.split(split_regex, string) if (x!= '' and x.lower() not in stopwords)]
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.flatMap(lambda s: s[1]).count()
...
def tf(tokens):
    ...
    tokenDict = {}
    for t in tokens:
        if t not in tokenDict.keys():
            tokenDict[t] = 1
        else:
            tokenDict[t] = tokenDict[t] + 1
    for k, v in tokenDict.items():
        tokenDict[k] = float(tokenDict[k])/len(tokens)
    return tokenDict
...
avgSimDups = float(trueDupsRDD.map(lambda x: x[0]).sum())/trueDupsCount

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = float(nonDupsRDD.map(lambda x: x[1]).sum())/nonDupsRDD.count()
...
def invert(record):
    ...
    """
    pairs = [(t, record[0]) for t in record[1].keys()]
    return (pairs)
